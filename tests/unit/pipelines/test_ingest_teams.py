...
# fixture alone WITHOUT executing ``ingest_teams.run``.
#
...
#: players and schedule suites already assert it in this exact shape — so it is
#: a published observability contract, not a captured snapshot. Built from
#: :data:`config.CSV_TEAMS` so renaming the artifact stays a single-point edit.
...
def test_run_writes_header_only_artifact_and_marks_checkpoint_for_empty_rowset(
    ...
    because a season whose team aggregates are momentarily unavailable upstream
    must still refresh the artifact instead of leaving yesterday's rows on disk
    looking current.
    ...
    zero-team denominator — the only ``/`` operators there compose
...
def test_rule5_write_failure_propagates_and_leaves_checkpoint_unmarked(
    ...
    teams pipeline must let that exception escape rather than absorb it.
