...
#:
#: ``ingest_lineups`` performs no filtering, no aggregation and no
...
# ---------------------------------------------------------------------------
def test_run_writes_exact_row_count_and_metric_value_for_lineups(
    recording_client,
    recording_writer,
    recording_checkpoint,
    sample_single_table_payload,
    ...
    * A "fix" to ``_ensure_season_column`` that suppressed the insertion,
    ...
    assert len(writer.writes) == 1, (
        "exactly one lineups artifact must be written before its row count can "
        f"be inspected; observed {len(writer.writes)} writes: {writer.writes!r}"
    )
    write = writer.writes[0]
    assert write["rows"] == EXPECTED_LINEUPS_ROWS, (
        f"lineups artifact must carry exactly {EXPECTED_LINEUPS_ROWS} rows (one "
        "rowSet entry each for Jokić, Dončić and Tatum => 1 + 1 + 1 = "
        f"{EXPECTED_LINEUPS_ROWS}); got rows={write['rows']} for artifact "
        f"{write['name']!r}"
    )
    ...
    assert row_incs[0].kwargs["n"] == EXPECTED_LINEUPS_ROWS, (
        "row-written counter must be incremented by the length of the frame "
        f"handed to the writer, i.e. n={EXPECTED_LINEUPS_ROWS}; got "
    ...
    assert int(df["PLAYER_ID"].nunique()) == EXPECTED_DISTINCT_PLAYER_IDS, (
        f"the {EXPECTED_LINEUPS_ROWS} written rows must span exactly "
    ...
    assert list(df.columns) == EXPECTED_LINEUPS_COLUMNS, (
        f"writer must receive columns {EXPECTED_LINEUPS_COLUMNS!r} — a lowercase "
...
def test_run_writes_header_only_artifact_and_marks_checkpoint_for_empty_rowset(
    ...
    * Substituting a bare ``pd.DataFrame()`` for the normalizer's
    ...
    # lineups pipeline receives a table whose ``rowSet`` is empty.
    ...
    write = writer.writes[0]
    assert write["name"] == config.CSV_LINEUPS, (
        f"the header-only artifact must still be named {config.CSV_LINEUPS!r}; "
        f"got {write['name']!r}"
    )
    ...
    assert checkpoint.marks == [(config.DOMAIN_LINEUPS, _EXPECTED_KEY)], (
        ...
        f"must mark (DOMAIN_LINEUPS, {_EXPECTED_KEY!r}) exactly once; observed "
    ...
    # filtering on pipeline="ingest_lineups", artifact="lineups.csv" must still
    # see the zero. A mutation that relabelled the empty-path increment (a
    # distinct "empty" artifact name, a dropped label, or legacy domain/file
    # label keys) would silently strand it outside every existing query while
    # the n=0 assertion above still passed.
    assert row_incs[0].args[1] == EXPECTED_ROW_COUNT_LABELS, (
...
def test_rule5_write_failure_propagates_and_leaves_checkpoint_unmarked(
    ...
    * Hoisting ``met.inc("pipeline_rows_written_total", ...)`` ABOVE
    ...
    assert metrics_mock.inc.call_args_list == [], (
        "the lineups pipeline emits exactly one counter and it is downstream of "
